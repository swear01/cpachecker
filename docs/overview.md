...
CPAchecker is a configurable software verifier for C programs, used in SV-COMP competitions. This fork adds **VGuide (Vocabulary-Guided CEGAR)**: an LLM-in-the-loop extension with implemented predicate and termination-ranking candidate paths. The current research entry point is the authenticated hard-218 manifest lineage; the stopped current checkpoint contains 184 matched pairs, with 66 arm executions absent. This is partial trend evidence, not formal timing or publication data. The LLM only proposes candidates/search spaces (Tier S) or controls resource/routing decisions (Tier R). No Tier X (direct LLM verdict or unverified assumption) is allowed.
