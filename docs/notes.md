...
- **Source selection and prompt accounting (#74/#109).** `ContextPackBuilder` enforces 300,000 UTF-16 source units including file headers/newlines/omission markers. Whole small files pass through when they fit; >100K files are sliced using file-local origin lines. On overflow, deterministic remaining-file shares prevent earlier files from consuming the entire source budget. Selection tries assertion + loop/path + declarations, then omits declarations, then non-assertion ranges, then a marked bounded head. Missing files are marked individually. Within slices, long source lines are truncated at 400 units; constant initializer values may be elided, so retained declarations do **not** imply retained values or complete source semantics. Assertion summaries use the first recognized balanced `__VERIFIER_assert` call, skipping standard helper declarations/comments; an empty summary does not mean `reach_error` or its guard is absent from retained source. Comment/literal masking preserves UTF-16 offsets and physical line breaks, including escaped string continuations.
- **Exact request accounting (#109).** Dump schema 12 retains legacy `prompt_chars`/`prompt_components`; those are log-text/source-payload measurements, not a reconciled token budget. Additive `prompt_accounting` (`prompt-accounting-v1`) partitions exact system/user text, user components (including hints/profile/history/outcomes/native/repair), and the 15 log-only role-delimiter units into UTF-16 units, Unicode code points and UTF-8 bytes. `context_selection` records source markers and CE relation limits; the standalone trace is unused, while structured trace is inside `ce_summary`. Context construction unwraps production ARG states before extracting structured trace locations. Per-call indexed `prompt_path` and `prompt_messages_path` prevent ensemble overwrite. Completion reservation is the effective `max(256, configured)` token cap; absent API usage, tokenizer/window limit and preflight window fit remain null/unknown. Do not infer tokens or provider headroom from characters. `PromptAuditTest` writes raw synthetic fixtures/formulas, retained source, exact messages/request and audit JSON when `-Dvguide.promptAuditDir=<new sibling experiments directory>` is set. Context corrections are a separate replayable development variant; never patch a frozen #180/#100/#102 prompt or infer benchmark gains from this model-free audit.
