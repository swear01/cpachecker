...
- **Source selection and prompt accounting (#74/#109).** `ContextPackBuilder` enforces 300,000 UTF-16 source units including file headers/newlines/omission markers. Whole small files pass through when they fit; >100K files are sliced using file-local origin lines. On overflow, deterministic remaining-file shares prevent earlier files from consuming the entire source budget. Selection tries assertion + loop/path + declarations, then omits declarations, then non-assertion ranges, then a marked bounded head. Missing files are marked individually. Within slices, long source lines are truncated at 400 units; constant initializer values may be elided, so retained declarations do **not** imply retained values or complete source semantics. Assertion summaries use the first recognized balanced `__VERIFIER_assert` call, skipping standard helper declarations/comments; an empty summary does not mean `reach_error` or its guard is absent from retained source. Comment/literal masking preserves UTF-16 offsets and physical line breaks, including escaped string continuations. **Known limitation:** line numbers are per-file; slicing assumes a single source file (multi-file programs may misalign ranges).
