...
- **CE-visible head priority (issue #183).** The candidate policy prioritizes useful splits at `L@N` entries in the current counterexample summary. Such entries have a trace block available to validation; summary caps and relation deduplication can omit other visited heads, so omission does not exclude a head. This wording changes ranking guidance only; context extraction and consumer validation remain unchanged.
