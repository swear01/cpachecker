...
- **實驗執行中禁止 rebuild classes / 修改 repo 檔案（since 2026-08-16）。** NFS 共享 repo：執行中的 CPA JVM 動態載入 class，`rm -rf classes && ant build` 或 checkout 會讓執行中的 JVM 出現 `NoClassDefFoundError: ...$1`（匿名 class 不一致）→ 整個 run 212/224 crash（Flash low 300s run 因此作廢）。也禁止改被執行中的 script（bash 逐段讀取 → `unexpected EOF`，arm-2/stock 各中招一次）。**先等所有 run 結束再改 code/rebuild。**
