...
- **官方 expected verdict 是 evaluation ground truth。** Issue #54 的 12 個 mismatch 必須計入 wrong；`scripts/vguided-cegar/official_label_conflicts.txt` 只提供 overflow/UB 與 dataset-risk 診斷標籤，不得作為 wrong gate exemption。
- **LM-helping 600s cohort（Issue #92 exclusion）。** 先以 218 題（224 減去 `docs/vguided-cegar/benchmark_sets/core_only_issue92_excluded.list` 的六個已知 MathSAT symbol-conflict task）比較 stock 與 Flash non-thinking augmented；六題保留 raw history，不納入 mechanism cohort。舊 600s runs 的「無 new solve」結論受 provider failure、commit/route 差異污染，不能當作正式結論。
