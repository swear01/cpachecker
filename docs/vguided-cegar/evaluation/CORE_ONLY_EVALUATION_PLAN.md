...
- 0 wrong is a hard gate; interrupted/invalid attempts remain visible, never silently retried. On SIGINT/SIGTERM, capture sends TERM to the verifier group, waits up to 10 seconds for the leader, KILLs remaining group members and reaps the leader before propagating the original signal. Repeated signals cannot bypass cleanup; SIGKILL cannot be handled. The raw partial log is retained with an `interrupted` execution sidecar containing the actual child exit/signal, elapsed time and log hash; this is an infrastructure failure, even if the verifier printed a result. A missing or corrupt sidecar remains invalid evidence. Preserve partial/corrupt evidence and use a fresh output directory for an explicitly recorded recovery. Same-directory resume requires unchanged provenance and skips records that parse as JSON with the expected task and captured command, including failures. This is only a partial check: run the smoke checker separately to validate referenced logs, execution sidecars and hashes before accepting evidence.

### 5. Held-out core-only evaluation

Run Stock-Core once for all 224 tasks. Verify completion, manifest/config hashes, and verdict soundness before running Augmented-Core once for the same 224 tasks. Do not retry individual tasks as a hidden performance fallback. Preserve interrupted or invalid attempts and record any recovery explicitly under a fresh output directory when partial/corrupt evidence exists. Same-directory resume checks provenance (arm/commit/config/manifest/timelimit/model/thinking) and record JSON/task/command identity, preserves failure records, and starts only tasks with no prior evidence; it does not overwrite incomplete attempts. Resume alone does not establish artifact integrity: the separate smoke checker must verify logs, execution sidecars and hashes before acceptance.
