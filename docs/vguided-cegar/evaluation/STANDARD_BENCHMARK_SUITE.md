...
> **不是** SV-COMP 官方池。舊稱「官方 20 題」= Legacy timeout-context 池（v1，**100% NO_SPURIOUS**），見本機 `archive/vguided-docs/evaluation/NO_SPURIOUS_STATISTICS.md`。
...
（排程預設已寫入 `vguide.properties`；LLM 行為見 `logs` 或 `verify-pack` 的 `cpa.log`。）
...
./scripts/vguided-cegar/run.sh cpa --set sample              # experiments/sample_vguide/
./scripts/vguided-cegar/run.sh cpa --set sample --mode stock # experiments/sample_stock/
