...
排程與 LLM 預設見 `config/vguide.properties`（`every_n=72`，**`llmSamplesPerCall=1`**，**predicate budget 3–6**）。見 [LLM_ENSEMBLE.md](llm/LLM_ENSEMBLE.md)、[PREDICATE_BUDGET.md](llm/PREDICATE_BUDGET.md)。覆寫例：
...
# 可選 ensemble（第 2+ 輪 LLM；#1 仍 1 draw）
./scripts/vguided-cegar/run.sh cpa --set sample -- \
  --option vguide.llmSamplesPerCall=3 --option vguide.llmSampleParallelism=3

# 現行 noL3 實驗 config（adaptive budget + freq10/n24；thinking 請設 disabled）
...
./scripts/vguided-cegar/run.sh cpa --set full_scalar --ablation no-l3 --parallel 8 --timelimit 300 \
  --out output/vguide/experiments/full_scalar_vguide_noL3_freq10_n24_adaptive_<date>
```
