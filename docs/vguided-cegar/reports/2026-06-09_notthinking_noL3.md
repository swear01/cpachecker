...
> **後續**：adaptive budget + freq10/n24 已 **150 solved / PAR-2 192s**（+13 vs 本 run）→ 見 [`2026-06-10_freq10_n24_adaptive_noL3.md`](2026-06-10_freq10_n24_adaptive_noL3.md)。
...
1. **主線配置（已由 adaptive 取代）：** 見 `config/vguide-experiment-freq10-n24.properties` + `VGUIDE_LLM_THINKING=disabled`。
2. **本 run 定位：** 驗證 thinking off 可行（+21 vs stock）；數量/頻率提升見 adaptive 報告。
