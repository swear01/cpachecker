...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 CPACHECKER_DIR SV_BENCHMARKS_DIR BENCHEXEC_DIR CAP16_FORMAL_OUTPUT OUTPUT_DIR" >&2
  exit 2
fi
...
FORMAL_PHASE_A="$FORMAL_OUTPUT/input/evidence"
FORMAL_MANIFEST="$FORMAL_PHASE_A/summary/candidate-manifest-analysis-survivors.json"
FORMAL_DEFINITION="$FORMAL_OUTPUT/generated/hard-case-candidates.xml"
FORMAL_PLAN_1="$FORMAL_OUTPUT/repetition-1-plan.json"
FORMAL_PLAN_2="$FORMAL_OUTPUT/repetition-2-plan.json"
FORMAL_CHECK=$(mktemp -d /var/tmp/vguide-cap16-formal-check.XXXXXX)
trap 'rm -rf -- "$FORMAL_CHECK"' EXIT
run_python_script "$FORMAL_DATASET" validate-cap16-phase-a \
  --phase-a-output "$FORMAL_PHASE_A" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR"
run_python_script "$FORMAL_DATASET" summarize-cap16-formal \
  --phase-a-output "$FORMAL_PHASE_A" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR" \
  --manifest "$FORMAL_MANIFEST" \
  --benchmark-definition "$FORMAL_DEFINITION" \
  --repetition-plan "$FORMAL_PLAN_1" \
  --repetition-plan "$FORMAL_PLAN_2" \
  --output-dir "$FORMAL_CHECK" --hard-threshold 200
for candidate in "$FORMAL_CHECK"/*; do
  cmp -- "$candidate" "$FORMAL_OUTPUT/summary/$(basename "$candidate")"
done
rm -rf -- "$FORMAL_CHECK"
trap - EXIT
run_python_script "$FORMAL_DATASET" validate-formal-closure \
  --output-root "$FORMAL_OUTPUT" \
  --manifest "$FORMAL_MANIFEST" \
  --benchmark-definition "$FORMAL_DEFINITION" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR" \
  --host athena --mode cap16 \
  --repetition-plan "$FORMAL_PLAN_1" \
  --repetition-plan "$FORMAL_PLAN_2" \
  --require-complete
...
if [[ "$RESUMING" == false ]]; then
  ...
  run_python_script "$SCRIPT_DIR/dataset.py" package-cap16-probe-input \
...
run_python_script "$DATASET_PY" validate-cap16-probe-input \
  --probe-input "$OUTPUT_DIR/input/formal" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR"
if [[ "$RESUMING" == true ]]; then
  EXPECTED_INPUT=$(mktemp -d /var/tmp/vguide-cap16-probe-input.XXXXXX)
  trap 'rm -rf -- "$EXPECTED_INPUT"' EXIT
  run_python_script "$DATASET_PY" package-cap16-probe-input \
    --formal-output "$FORMAL_OUTPUT" \
    --sv-benchmarks "$SV_BENCHMARKS_DIR" \
...
PROBE_MANIFEST="$OUTPUT_DIR/input/formal/candidate-manifest-cap16-probe.json"

if [[ -e "$OUTPUT_DIR/summary/.complete" ||
  -L "$OUTPUT_DIR/summary/.complete" ]]; then
  run_python_script "$DATASET_PY" validate-cap16-probe-closure \
...
if [[ ! -f "$OUTPUT_DIR/generated/cegar-eligibility.xml" ]]; then
  run_python_script "$DATASET_PY" render-cap16-probe \
    --probe-input "$OUTPUT_DIR/input/formal" \
    --sv-benchmarks "$SV_BENCHMARKS_DIR" \
    --property-file "$SV_BENCHMARKS_DIR/c/properties/unreach-call.prp" \
    --output-dir "$OUTPUT_DIR/generated"
fi
authenticate_probe_definition true "$OUTPUT_DIR/generated" render-cap16-probe \
  --probe-input "$OUTPUT_DIR/input/formal" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR" \
  --property-file "$SV_BENCHMARKS_DIR/c/properties/unreach-call.prp"
...
run_probe_attempt() {
  ...
  local definition=$3
  local output=$4
  local name="hard-case-dataset-v2-cap16-cegar-probe-athena-$label"
  ...
  local -a evidence=(
    ...
    --sv-benchmarks "$SV_BENCHMARKS_DIR" --host athena
    --mode cap16-probe --label "$label" --role "$role" --repetition 1
  ...
  run_python_script "$DATASET_PY" write-formal-process-descriptor \
    --output-root "$OUTPUT_DIR" --mode cap16-probe --label "$label" \
    --host athena --name "$name" --definition "$definition" \
...
while [[ $("$PYTHON_BIN" -I -c \
  ...
  if [[ ! -f "$definition" ]]; then
    run_python_script "$DATASET_PY" render-cap16-probe-replacement \
      --probe-input "$OUTPUT_DIR/input/formal" \
      --sv-benchmarks "$SV_BENCHMARKS_DIR" \
...
if [[ ! -f "$OUTPUT_DIR/probe-plan.json" ]]; then
  ...
  run_python_script "$DATASET_PY" cap16-probe-plan "${plan_args[@]}"
fi

SUMMARY_STAGE="$OUTPUT_DIR/summary.staging"
rm -rf -- "$SUMMARY_STAGE"
run_python_script "$DATASET_PY" cap16-probe-summary \
  --probe-input "$OUTPUT_DIR/input/formal" \
  --sv-benchmarks "$SV_BENCHMARKS_DIR" \
